...
namespace FabricaCEAPE.Datos
{
    class DatosDepartamento : Conexion
    {
        ...
        public static bool existeDepartamentoN(int id, string nombre)
        {
            SqlConnection cnn = new SqlConnection(Conexion.Connection);
            //abro la conexion
            cnn.Open();
            ...
            //Creo el comando sql a utlizar
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM Departamentos where activo = 1 and id = @id and nombre = @nombre");
